...
import { ArrowRight, Images, FolderOpen } from "lucide-react"
...
function FolderTile({ category }: FolderTileProps) {
  return (
    <motion.div
      variants={fadeInUp}
      className="group relative flex flex-col h-full rounded-2xl bg-white border border-neutral-200/80 shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5"
    >
...
export function GallerySection() {
  ...
  const previewPhotos: GalleryImage[] = React.useMemo(() => {
    return galleryCategoriesData.flatMap((cat) => cat.images.slice(0, 2))
  }, [])
      ...
      <div className="container-hero relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          <p className="text-neutral-600 text-sm sm:text-base leading-relaxed font-sans max-w-2xl mx-auto">
            Browse event photo collections categorized by wedding celebrations, milestone occasions, corporate summits, and promo posters.
          </p>
        </div>
        ...
        <motion.div
          ...
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 items-stretch mb-14 sm:mb-16"
        >
