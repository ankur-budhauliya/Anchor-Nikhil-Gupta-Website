...
function StatCard({ item, index, isInView, prefersReducedMotion }: StatCardProps) {
  ...
  return (
    <motion.div
      variants={fadeInUp}
      className={`flex flex-col items-center text-center px-3 sm:px-4 py-7 sm:py-9 lg:py-10 relative transition-colors duration-300 hover:bg-white/[0.02] ${colSpanClass} ${borderClasses}`}
      aria-label={screenReaderText}
    >
      {/* Metric Display */}
      <div className="font-display text-4xl sm:text-5xl lg:text-5xl xl:text-6xl font-bold text-white tracking-tight leading-none mb-3 flex items-baseline justify-center">
        {item.prefix && <span className="text-gold mr-0.5">{item.prefix}</span>}
        {item.numericValue !== undefined ? (
          <span>{count}</span>
        ) : (
          <span className="text-4xl sm:text-5xl lg:text-[48px] tracking-wide font-display text-white">
            {item.value}
          </span>
        )}
        {item.suffix && <span className="text-gold ml-0.5">{item.suffix}</span>}
      </div>

      {/* Decorative gold accent hairline */}
      <div
        className="h-[2px] w-8 bg-gradient-to-r from-transparent via-gold to-transparent mb-2.5 opacity-80"
        aria-hidden="true"
      />
...
export function Stats() {
  ...
  return (
    <section
      ...
      className="relative w-full bg-[#4E0A16] border-y border-white/10 py-10 sm:py-12 lg:py-14 overflow-hidden"
    >
      ...
      <div className="container-hero relative z-10">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <p
            id="stats-heading"
            className="font-sans text-xs sm:text-sm font-bold tracking-[0.25em] text-gold uppercase"
          >
            THE EXPERIENCE BEHIND THE ENERGY
          </p>
          <div
            className="h-[2px] w-12 bg-gold/70 mx-auto mt-2 rounded-full"
            aria-hidden="true"
          />
        </div>

        {/* 5-Column Statistics Matrix */}
        <motion.div
          ref={containerRef}
          variants={staggerContainer}
          initial="hidden"
          animate={isInView ? "visible" : "hidden"}
          className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 rounded-2xl bg-white/[0.03] backdrop-blur-xs border border-white/10 shadow-2xl overflow-hidden"
        >
